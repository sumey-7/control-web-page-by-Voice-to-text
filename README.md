...
1. 📄 **`db.php`**: Database connection script.
2. 🛠️ **`setup.sql`**: SQL script to create the `robot_state` table (executed once in phpMyAdmin).
3. 🔄 **`update_command.php`**: Receives commands (from buttons or voice), maps them, and updates the database.
4. 📡 **`get_state.php`**: Fetches the current robot state from the database.
5. 🌐 **`index.html`**: Interactive user interface featuring control buttons and voice recognition.

---

## 🎙️ Voice Control Feature
